...
class AddressCdcConsumer(
        ...
        try {
            span.makeCurrent().use {
                ...
                try {
                    ...
                    logger.info("Received address CDC event")

                    val (operation, outcome) = when {
                        value == null -> {
                            logger.info("Processing address tombstone message")
                    ...
                    logger.info("Address CDC event processed successfully")
            ...
            logger.error("Error processing address CDC event: {}", e.message, e)
    ...
    private fun processEvent(
        ...
        return if (event.isDelete()) {
            ...
            addressMongoService.upsert(event, kafkaOffset, kafkaPartition).block()
